...
export const WIDGET_DEFINITIONS: WidgetDefinition[] = [
  ...
  { key: "true-false", name: "True or false", category: "Knowledge check", description: "A statement with an answer and explanatory feedback.", version: "1.0.0", template: `<section class="workpath-knowledge-check" style="border:1px solid #b9c8da;border-radius:6px;margin:20px 0;padding:20px"><h3 style="margin-top:0">{{question}}</h3><ul><li>True</li><li>False</li></ul><details><summary style="cursor:pointer;font-weight:700">Check answer</summary><p><strong>{{answer}}</strong></p>{{{feedback}}}</details></section>`, parameters: [
  ...
  { key: "single-choice", name: "Single-answer knowledge check", category: "Knowledge check", description: "A multiple-choice question with one correct answer.", version: "1.0.0", template: `<section class="workpath-knowledge-check" style="border:1px solid #b9c8da;border-radius:6px;margin:20px 0;padding:20px"><h3 style="margin-top:0">{{question}}</h3><ol type="A">{{#options}}<li>{{text}}</li>{{/options}}</ol><details><summary style="cursor:pointer;font-weight:700">Check answer</summary><p><strong>Correct answer:</strong> {{correctAnswer}}</p>{{{feedback}}}</details></section>`, parameters: [
  ...
  { key: "multiple-choice", name: "Multiple-answer knowledge check", category: "Knowledge check", description: "A question where more than one response may be correct.", version: "1.0.0", template: `<section class="workpath-knowledge-check" style="border:1px solid #b9c8da;border-radius:6px;margin:20px 0;padding:20px"><h3 style="margin-top:0">{{question}}</h3><ul>{{#options}}<li>{{text}}</li>{{/options}}</ul><details><summary style="cursor:pointer;font-weight:700">Check answers</summary><p><strong>Correct answers:</strong> {{correctAnswers}}</p>{{{feedback}}}</details></section>`, parameters: [
  ...
  { key: "flip-cards", name: "Flip cards", category: "Interactive", description: "Prompt-and-reveal cards with accessible disclosure controls.", version: "1.0.0", template: `<section class="workpath-flip-cards" style="display:flex;flex-wrap:wrap;gap:16px;margin:20px 0">{{#cards}}<details style="border:1px solid #c6d1df;border-radius:6px;box-sizing:border-box;flex:1 1 220px;padding:16px"><summary style="cursor:pointer;font-weight:700">{{frontTitle}}</summary>{{{frontBody}}}<div style="border-top:1px solid #d9e1ea;margin-top:12px;padding-top:12px"><strong>{{backTitle}}</strong>{{{backBody}}}</div></details>{{/cards}}</section>`, parameters: [
    { name: "cards", title: "Cards", type: "repeatable", min: 1, max: 12, initial: 3, itemLabel: "Card", fields: [{ name: "frontTitle", title: "Front title", type: "textfield", default: "Prompt {{i}}" }, { name: "frontBody", title: "Front content", type: "richtext", default: "<p>Add the prompt.</p>" }, { name: "backTitle", title: "Back title", type: "textfield", default: "Answer {{i}}" }, { name: "backBody", title: "Back content", type: "richtext", default: "<p>Add the reveal.</p>" }] }
  ] },
  { key: "hotspot-image", name: "Hotspot image", category: "Interactive", description: "An annotated image with positioned numbered callouts.", version: "1.1.0", template: `<figure class="workpath-hotspot" style="margin:20px 0"><div style="position:relative">{{#imageUrl}}<img src="{{{imageUrl}}}" alt="{{altText}}" style="display:block;height:auto;max-width:100%;width:100%">{{/imageUrl}}{{^imageUrl}}{{#altText}}<div role="img" aria-label="{{altText}}" style="background:#ffffff;border:2px dashed #bdc8d6;color:#52647b;padding:40px;text-align:center">{{altText}}</div>{{/altText}}{{^altText}}<div role="img" aria-label="Placeholder image" style="background:#ffffff;border:2px dashed #bdc8d6;color:#65728a;padding:40px;text-align:center">Placeholder image</div>{{/altText}}{{/imageUrl}}{{#imageUrl}}{{#hotspots}}<span aria-hidden="true" style="background:#245fa6;border:2px solid #fff;border-radius:50%;color:#fff;font-weight:700;left:{{x}}%;padding:5px 9px;position:absolute;top:{{y}}%;transform:translate(-50%,-50%)">{{number}}</span>{{/hotspots}}{{/imageUrl}}</div><figcaption><ol>{{#hotspots}}<li><strong>{{title}}</strong>{{{body}}}</li>{{/hotspots}}</ol></figcaption></figure>`, parameters: [
