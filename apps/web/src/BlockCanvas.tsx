...
export function BlockCanvas({ blocks, assets, assetUrl, onChange, onUpload }: { blocks: ContentBlock[]; assets: AssetRecord[]; assetUrl: (asset: AssetRecord) => string; onChange: (blocks: ContentBlock[]) => void; onUpload: (blockId: string, parameterName: string, file: File) => Promise<void> }) {
  ...
  return <section className="block-builder" aria-label="Topic blocks">
    ...
    {pickerOpen && <div className="block-picker"><button onClick={() => add(makeRichText())}><strong>Rich text</strong><span>Formatted text using TinyMCE</span></button>{WIDGET_DEFINITIONS.map((definition) => <button key={definition.key} onClick={() => add(createWidgetBlock(definition.key))}><strong>{definition.name}</strong><span>{definition.description}</span><small>{definition.category}</small></button>)}</div>}
...
function ParameterEditor({ parameter, value, onChange, id, assets, assetUrl, onUpload }: { parameter: WidgetParameter; value: unknown; onChange: (value: unknown) => void; id: string; assets: AssetRecord[]; assetUrl: (asset: AssetRecord) => string; onUpload: (file: File) => Promise<void> }) {
